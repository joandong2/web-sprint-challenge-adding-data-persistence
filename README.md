...
1. Explain the difference between `Relational Databases` and `SQL`.
   == Relational database is a database that stores and provides access to data points that are related to one another while SQL, Structured Query Language, used for storing, retrieving, managing or manipulating the data inside a realtional database

2. Why do tables need a `primary key`?
   == It serves as a unique identifier, in which we can use to reference or form a relationship to other entities/table
